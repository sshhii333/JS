...
let arr = [23, 'привет', 0, , -2, 56, 'ыыы', 11];
...
for (let i = 0; i < arr.length; i++) {
	if (typeof arr[i] === 'number' && arr[i] > 0 ) plus++;
	if (typeof arr[i] === 'number' && arr[i] < 0 ) minus++;
	if (typeof arr[i] === "string") text++;
};

console.log(plus, minus, text);
